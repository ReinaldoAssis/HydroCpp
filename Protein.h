...
struct protein {
  ...
  protein(int _length, vector2 initialPosition = vector2(0, 0),
          bool randomStart = false, bool _randomizeSequences = false,
          char _emptyChar = '*') {
    length = _length;
  ...
  enum PRINT_MODE { PRINT_MODE_NORMAL, PRINT_MODE_INDEXED };

  void initialize(int _length, bool randomStart = false,
                  bool _randomizeSequences = false);
  void append(std::vector<aminoacid> sequence);
  void print(PRINT_MODE mode = PRINT_MODE_NORMAL);

  // Checks if position + move respects the boundaries of the matrix and if the
  ...
  // @returns a valid delta movement if possible, otherwise returns
  // VECTOR2_INVALID
